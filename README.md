...
- [architecture.md](architecture.md)：系统定位、核心闭环、数据对象、Agent 分工、监测链路和已知边界。
- [docs/RELEASE.md](docs/RELEASE.md)：本地打包、GitHub Actions 桌面包构建和 GitHub Release 发布步骤。
...
新环境默认不再内置固定密码账号。数据库中没有任何用户时，登录页会进入初始化模式，请自行创建本地管理员账号。

开发或演示环境如果确实需要自动准备项目负责人账号，可以显式设置以下环境变量：
